...
// The load-time merge pipeline (profile application, deep merge, managed and
// requirements overlays) lives in the `merge` leaf module; the parent pulls
// the pipeline entry points back in so `Config::load` and the config tests
// keep resolving them unchanged (#5586).
mod merge;
#[cfg(test)]
use merge::merge_config;
use merge::{
    apply_managed_overrides, apply_profile, apply_requirements, load_sibling_exec_policy_engine,
    load_single_config_file, warn_on_misplaced_top_level_keys,
};
...
// Credential persistence (secret-store vs config-file writes, save/clear
// surfaces, key probes) lives in the `credentials` leaf; the parent
// re-exports the names so every `crate::config::<name>` path keeps
// resolving unchanged (#5586).
mod credentials;
pub use credentials::*;
use credentials::{
    bound_provider_api_key_env_name, missing_provider_api_key_message, provider_config_env_api_key,
    provider_config_table_name, provider_env_api_key, provider_secret_store_api_key_with_mode,
    provider_secret_store_slot, provider_uses_oauth_credentials,
    secret_slot_save_marker_on_shared_slot, user_global_config_api_key,
    warn_on_config_api_key_shadowing, write_config_file_secure,
};
#[cfg(test)]
use credentials::{config_api_key_shadow_warning, provider_config_is_explicit};
