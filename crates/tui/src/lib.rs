...
// The clap surface (arg structs + CLI-input interpretation helpers) lives
// in `cli_args`; the glob re-export keeps `crate::Cli`, `crate::Commands`,
// and every later use in this file resolving unchanged (#5586).
mod cli_args;
...
// The doctor CLI report layer (human + JSON reporters, legacy-state and
// session-recovery types, per-check helpers) lives in `doctor_cli`; the
