...
- `tui.right_click_menu` (bool, optional, default `true`): whether right-click inside the TUI opens Codewhale's own context menu. Terminals that open their native menu on the same gesture — notably the UOS (deepin) default terminal — show two menus with this on; set `false` or run with `--no-right-click-menu` to hand right-click to the terminal's menu. Wheel scrolling, transcript selection, and scrollbar dragging are unaffected; they follow `tui.mouse_capture`.
